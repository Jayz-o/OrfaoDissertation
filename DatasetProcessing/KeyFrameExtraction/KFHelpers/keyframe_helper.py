...
def get_sil_for_k_tf(k, feature_array, spherical=False, repeat=3):
    try:
        import faiss
        print(f"Clustering: {k}")
        # print(MatrixStats(feature_array).comments)
        kmeans = faiss.Kmeans(d=feature_array.shape[1], k=k, gpu=True, verbose=False, spherical=spherical, nredo=repeat)
        # kmeans.cp.nredo = repeat
...
def reduce_features(pf_dic):
    from sklearn.preprocessing import StandardScaler
    from sklearn.decomposition import PCA
    sc = StandardScaler()
    features = np.stack(list(pf_dic.values()))
    features_std = []
    for i in range(len(features)):
        features_std.append(sc.fit_transform(features[i].reshape(-1,1)).squeeze())
    features_std = np.array(features_std)
    pca = PCA()
    X_train_pca = pca.fit_transform(features_std)
    decomposed_dic = {}
    for index,  key in enumerate(pf_dic):
        decomposed_dic[key] = X_train_pca[index]
    return decomposed_dic
def find_optimal_k(frame_directory, save_root, dataset_root, max_k=None,
    ...
    if path_feature_dic_file is None:
        ...
        path_feature_dic_file = reduce_features(path_feature_dic_file)
        ...
        if optimal_k < 0:
            ...
            # get the old datasets csv
            info_csv_path = os.path.join(dataset_root, frame_directory, INFO_CSV_NAME)

            # read the csv and update the frames
            info_csv = pd.read_csv(info_csv_path)

            # fix csv structure
            if "frames" in info_csv.columns:
                info_csv.rename(columns={'frames': 'frames_present'}, inplace=True)

            info_csv["video_frames"] = info_csv['frames_present']  # number of original frames used in clustering
            info_csv['frames_present'] = -1 # number of KF
            info_csv['optimal_k'] = optimal_k # number of KF
...
def extract_vgg_features(config):
    ...
    extract_features_from_paths(image_files=file_paths, save_root=config['save_root'], frame_dir=file_directory, force_redo=config['force_redo'],
...
def create_kf_dataset(dataset_root, save_root, tune_root, dataset_csv_name, mean_col_name, min_thresh_function, tune_extraction_gpu=0.5,
    ...
    if dataset_csv_location is None:
        raise TypeError(f"Could not find the csv file: {dataset_csv_location}")
    ...
    df = pd.read_csv(dataset_csv_location)
    df = df.sort_values('frames_present')
    # df = df.sort_values('frames_present', ascending=False)
    if all_video_files is None:
        all_video_files = df['directory_path'].tolist()
...
if __name__ == '__main__':
    ...
    # view_keyframe_reduction(os.path.join(save_root, dataset_csv_name), save_root, is_pdf=True)
    # category_mean_dic = get_category_mean_dic(save_root, dataset_name, mean_col_name, use_only_training_information)
    get_category_mean_dic(save_root,"siw", mean_col_name,False)
