...
def train_gan(project_root, dataset_name, training_folder_name, duration_kimg, num_gpu=1, is_sg2=False):
    ...
    try:
        ...
        if is_sg2:
            args = [f"--outdir={save_path}", f"--data={data_path}", "--mirror=1", "--cfg=stylegan2", "--batch=32",
                    "--gamma=32"]
        else:
